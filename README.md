...
In the `src/mytweets.models`, create the models that represent the key data to be parsed and persisted within your application.
For example, if you were connecting to Twitter, you would want a Tweet model as follows:

```java
// models/Tweet.java
package mytweets.models;
...
Open `src/mytweets/LoginActivity.java` and configure the `onLoginSuccess` method
